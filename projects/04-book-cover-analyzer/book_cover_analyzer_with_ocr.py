"""
Book Cover Analyzer - WITH OCR (Optical Character Recognition)
===============================================================
Three-stage pipeline:
Stage 1: Find and isolate the book cover from background
Stage 2: Detect text and image elements ON the book cover
Stage 3: READ the actual text using EasyOCR (NEW!)

CONCEPTS USED:
- Classical CV: Edge detection, contours, shape analysis
- Deep Learning: EasyOCR neural network for text recognition
- Hybrid approach: CV finds regions, AI reads text
"""
...
import easyocr
import warnings
warnings.filterwarnings('ignore')  # Suppress EasyOCR warnings
...
def read_text_with_ocr(book_image, regions, reader):
    """
    NEW! Use EasyOCR to READ the actual text from detected text regions.
    ...
    Returns:
        regions with added 'ocr_text' field
    """
    print("\n" + "="*60)
    print("STAGE 3: READING TEXT WITH OCR (Deep Learning)")
    print("="*60)
    print("[INFO] Using EasyOCR neural network to read text...")
    print("[INFO] This may take 1-2 seconds per region...")

    for idx, region in enumerate(regions, 1):
        # Only OCR text regions, skip images
        classification = region['classification']
        if "Text" not in classification and "Line" not in classification:
        ...
        try:
            ...
            # reader.readtext returns: [([x1,y1], [x2,y2], [x3,y3], [x4,y4]), text, confidence]
            result = reader.readtext(region_crop, detail=1)

            if result:
                # Combine all detected text in this region
                text_list = [detection[1] for detection in result]
                confidence_list = [detection[2] for detection in result]

                combined_text = " ".join(text_list)
                avg_confidence = sum(confidence_list) / len(confidence_list) if confidence_list else 0

                region['ocr_text'] = combined_text
                region['ocr_confidence'] = avg_confidence

                print(f"  Region #{idx}: \"{combined_text}\" (confidence: {avg_confidence:.2f})")
            else:
                region['ocr_text'] = "[empty]"
                region['ocr_confidence'] = 0.0
                print(f"  Region #{idx}: [no text detected]")

        except Exception as e:
            region['ocr_text'] = "[error]"
            region['ocr_confidence'] = 0.0
            print(f"  Region #{idx}: [OCR error: {str(e)[:50]}]")

    print(f"[SUCCESS] OCR completed on {len(regions)} regions")
    return regions
...
def print_detection_summary(regions, book_image):
    ...
    # Print text regions in table format with OCR results!
    print(f"\n📝 TEXT REGIONS DETECTED: {len(text_regions)}")
    print("="*120)
    print(f"{'#':<4} {'Text Content (OCR)':<50} {'Confidence':<12} {'Type':<25}")
    print("-" * 120)
    if text_regions:
        for idx, region in text_regions:
            classification = region['classification']
            ocr_text = region.get('ocr_text', '[not processed]')
            ocr_conf = region.get('ocr_confidence', 0.0)

            # Truncate long text
            if ocr_text and len(ocr_text) > 47:
                display_text = ocr_text[:44] + "..."
            else:
                display_text = ocr_text if ocr_text else "[empty]"

            conf_str = f"{ocr_conf:.0%}" if ocr_conf > 0 else "N/A"

            print(f"{idx:<4} {display_text:<50} {conf_str:<12} {classification:<25}")
    else:
...
def print_ocr_results(regions):
    """
    Print a simple, clean table showing what OCR read from the book cover.
    Separates text regions and image regions.

    Args:
        regions: List of detected regions with OCR results
    """
    print("\n" + "="*80)
    print("📖 WHAT THE OCR READ FROM THE BOOK COVER")
    print("="*80)
    ...
    for idx, region in enumerate(regions, 1):
        classification = region['classification']
        is_text = "Text" in classification or "Line" in classification

        if is_text:
            ocr_text = region.get('ocr_text', '[empty]')
            confidence = region.get('ocr_confidence', 0.0)
            if ocr_text and ocr_text not in ['[empty]', '[error]']:
                text_regions.append((idx, ocr_text, confidence))
        else:
            x, y, w, h = region['bbox']
            image_regions.append((idx, w, h))

    # Print text results
    print("\n📝 TEXT DETECTED AND READ:")
    print("-" * 80)
    if text_regions:
        print(f"{'#':<6} {'Text Read by OCR':<50} {'Confidence':<15}")
        print("-" * 80)
        for idx, text, conf in text_regions:
            conf_str = f"{conf*100:.0f}%" if conf > 0 else "N/A"
            print(f"Text {idx:<3} {text:<50} {conf_str:<15}")
        print(f"\nTotal: {len(text_regions)} text region(s) with readable content")
    else:
        print("No text detected or OCR couldn't read any text")

    # Print image results
    print("\n🖼️  IMAGES/ICONS DETECTED:")
    print("-" * 80)
    if image_regions:
        print(f"{'#':<6} {'Size':<20} {'Type':<30}")
        print("-" * 80)
        for idx, w, h in image_regions:
            print(f"Image {idx:<2} {w}×{h} pixels{'':<8} {'Icon/Graphic element':<30}")
        print(f"\nTotal: {len(image_regions)} image/icon region(s)")
    else:
...
def main():
    """
    Main function - Book cover analyzer WITH OCR!
    """
    ap = argparse.ArgumentParser(description="Book Cover Analyzer with OCR")
    ap.add_argument("-i", "--image", required=True,
                    help="Path to the book cover image")
    args = vars(ap.parse_args())

    print("="*60)
    print("BOOK COVER ANALYZER WITH OCR")
    print("Three-Stage Pipeline: Detect → Classify → Read Text")
    print("="*60)

    # Initialize EasyOCR (will download model on first run!)
    print("\n[INFO] Initializing EasyOCR...")
    print("[INFO] First run will download ~100MB language model")
    print("[INFO] This only happens once, then it's cached")
    ...
    # STAGE 3: READ TEXT WITH OCR! (NEW!)
    regions = read_text_with_ocr(book_image, regions, reader)

    # Print clear summary with OCR results
    print_detection_summary(regions, book_image)

    # Print simple OCR results table
    print_ocr_results(regions)
